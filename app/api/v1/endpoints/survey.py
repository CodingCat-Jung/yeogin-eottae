...
# --- 좌표 보강 (Google Places) ---
# enrich_one_recommendation(rec, collect_debug=False) 시그니처 권장
...
def _normalize_city_item(item: Any) -> Optional[Dict[str, Any]]:
    ...
    출력: {"city": str, "country": str, "reason": str, "schedule": [{day, activities:[{time,activity}]}]}
    """
    ...
    country = str(item.get("country") or "").strip()  # 없으면 빈 문자열
    ...
    return {"city": city, "country": country, "reason": reason, "schedule": norm_days}
...
@router.post("/recommend", summary="[RAG 적용] 설문 저장 + AI 추천 생성 (+좌표 보강)")
async def survey_and_recommend(
    ...
    debug: bool = Query(False, description="디버그 정보 포함 응답 여부"),  # /recommend?debug=1
):
    # 1) CSRF + 닉네임 보정
    session_csrf = request.session.get("csrf")
    if not x_csrf_token or not session_csrf or x_csrf_token != session_csrf:
        log.warning("CSRF invalid user=%s", user.get("nickname"))
    ...
    log.info("[REQ] recommend by user=%s", user.get("nickname"))

    # 2) 설문 저장
    saved_survey = survey_crud.create_survey(db=db, survey=survey)
    log.debug("saved survey_id=%s", getattr(saved_survey, "id", None))
    ...
    try:
        ...
        log.debug("prefs=%s", preferences_dict)
        ...
        if raw is None:
            log.error("AI recommendation empty")
        ...
        if not isinstance(raw, list):
            log.error("AI recommendation invalid format, type=%s", type(raw))
    ...
    # 4) 일정 후처리 (2차 강제 정규화 → guard → map 포맷)
    duration = preferences_dict.get("duration")
    days = _extract_days(duration)
    log.info("guarding schedule days=%s", days)
    ...
    for city in normalized:
        try:
            ...
            schedule_arr = _force_schedule_shape(schedule_arr, days)  # 최종 강제
            ...
            log.exception("schedule guard failed city=%s err=%s", city.get("city"), e)
            continue

    if not safe_data:
        log.error("all items invalid after guarding")
        raise HTTPException(status_code=502, detail="AI recommendation error: all items invalid after guarding")

    log.info("guarded cities=%d", len(safe_data))

    # 5) 좌표/경로 보강 (Google Places)
    try:
        enriched_list = await asyncio.gather(
            *(enrich_one_recommendation(dict(r), collect_debug=debug) for r in safe_data)
        )
        final_data = list(enriched_list)
        stats = [(len(d.get("allPlaces", [])), len(d.get("days", []))) for d in final_data]
        log.info("enriched items=%s stats(allPlaces,days)=%s", len(final_data), stats)
    except Exception as e:
        log.exception("geo enrich failed: %s", e)
        final_data = safe_data

    # 저장시 _debug 제거 (응답에만 포함)
    to_save = []
    ...
    resp = {
        ...
        "data": final_data if debug else to_save,  # debug=1이면 _debug 포함
