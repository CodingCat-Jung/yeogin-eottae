...
[일정 구성 가이드]
- 각 날은 보통 3~5개의 활동으로 구성한다.
- 각 활동은 "time"(HH:MM-HH:MM)과 "activity"(간결한 설명) 필드를 포함한다.
- 이동수단은 괄호로 표기한다.
- 하루 1회 '포토 스팟', 1회 '지역 음식/맛집', 1회 '문화/체험' 요소를 포함한다.
- **모든 식사 일정(점심/저녁)에는 반드시 실제 존재하는 식당 이름을 포함한다.**
- **식당은 구글 지도 기준 평점 4.5 이상인 곳을 우선 추천한다.**
- **조건에 맞는 식당이 부족하면 평점 4.3 이상을 허용한다.**
- 출력 activity에는 반드시 `"저녁: Ramiro ★4.6 (해산물 레스토랑, 도보)"` 같은 형식을 사용한다.
- 마지막 날은 공항 이동/체크아웃/귀국 준비를 반영한다.
- 활동 설명에는 과장/광고 문구를 피하고 실행 가능한 표현만 사용한다.
- 동선을 최적화하고 불필요한 이동을 최소화한다.

[출력 형식]
- 반드시 JSON만 반환한다. 자연어 문장, 설명, 마크다운 금지.
- 정확한 JSON 문법 사용, 쌍따옴표 필수, 후행 쉼표 금지.
